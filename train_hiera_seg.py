...
hiera_config        = model_params.get("hiera")

# Segmentation-specific parameters
num_classes         = hiera_config.get("num_classes", 2)
decoder_embed_dim   = hiera_config.get("decoder_embed_dim", 512)
decoder_depth       = hiera_config.get("decoder_depth", 8)
decoder_num_heads   = hiera_config.get("decoder_num_heads", 16)

# Core Hiera architecture parameters
input_size          = tuple(hiera_config.get("input_size", [224, 224]))
in_chans            = hiera_config.get("in_chans", 1)  # Typically 1 for X-ray data
embed_dim           = hiera_config.get("embed_dim", 96)
num_heads           = hiera_config.get("num_heads", 1)
stages              = tuple(hiera_config.get("stages", [2, 3, 16, 3]))
q_pool              = hiera_config.get("q_pool", 3)
q_stride            = tuple(hiera_config.get("q_stride", [2, 2]))
mask_unit_size      = tuple(hiera_config.get("mask_unit_size", [8, 8]))
mask_unit_attn      = tuple(hiera_config.get("mask_unit_attn", [True, True, False, False]))
dim_mul             = hiera_config.get("dim_mul", 2.0)
head_mul            = hiera_config.get("head_mul", 2.0)
patch_kernel        = tuple(hiera_config.get("patch_kernel", [7, 7]))
patch_stride        = tuple(hiera_config.get("patch_stride", [4, 4]))
patch_padding       = tuple(hiera_config.get("patch_padding", [3, 3]))
mlp_ratio           = hiera_config.get("mlp_ratio", 4.0)
drop_path_rate      = hiera_config.get("drop_path_rate", 0.0)
# Handle norm_layer - convert string to actual layer class
norm_layer_str      = hiera_config.get("norm_layer", "LayerNorm")
if isinstance(norm_layer_str, str):
    from functools import partial
    norm_layer = partial(getattr(nn, norm_layer_str), eps=1e-6)
else:
    norm_layer = norm_layer_str
head_dropout        = hiera_config.get("head_dropout", 0.0)
head_init_scale     = hiera_config.get("head_init_scale", 0.001)
sep_pos_embed       = hiera_config.get("sep_pos_embed", False)
...
auto_wrap_policy = partial(
    ...
    transformer_layer_cls={
        HieraBlock,  # Main transformer blocks in Hiera
        MaskedAutoencoderHiera,  # Decoder blocks if needed
    },
)
...
num_flops_per_token = estimate_flops_per_token(model, dummy_shape, hiera_patch_size, count_multiply_add_as = 2, includes_backward = True)
