...
def main(cfg: DictConfig):
    ...
    print(f"📝 Generating shared YAML config: {yaml_path}")
    ...
    if cfg.get('skip_preempt', False):
        try:
            ...
            print(f"🗑️  Removed preemptive checkpoint: {path_preempt}")
        except Exception as e:
            print(f"⚠️  Could not remove preemptive checkpoint: {e}")
    
    if os.path.exists(path_preempt):
        if cfg.train_config.checkpoint.get('path_chkpt_prev') is not None:
            try:
                os.remove(path_preempt)
                print(f"🗑️  Removed preemptive checkpoint (user specified path_chkpt_prev): {path_preempt}")
            except Exception as e:
                print(f"⚠️  Could not remove preemptive checkpoint: {e}")
