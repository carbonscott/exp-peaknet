...
mpirun -n 10 peaknet-pipeline-mpi \
    --batch_size 4 \
    --num_workers 1 \
    --config_path configs/s3df-issue10-atto-4bifpn512-1.0.yaml \
    --weights_path weights/s3df-issue10-atto-4bifpn512-1.0.bin \
    --dtype bfloat16 \
...
mpirun -n 10 peaknet-pipeline-write-to-cxi \
    ...
    --basename s3df-issue10-atto-4bifpn512-1.0 \
    --save_every 2 \
    --min_num_peak 15 \
...
ssh $HEAD_NODE "$SETUP_CMD && $JOB1_CMD &> inference_log/psana-ray-producer.s3df-atto-4bifpn512-1.0.log" &
JOB1_PID=$!
...
for node in $HEAD_NODE $WORKER_NODES; do
    ssh $node "$SETUP_CMD && $JOB2_CMD &> 'inference_log/peaknet-pipeline-mpi.s3df-atto-4bifpn512-1.0.${node}.log'" &
    JOB2_PIDS+=($!)
done
...
ssh $HEAD_NODE "$SETUP_CMD && $JOB3_CMD &> inference_log/peaknet-pipeline-write-to-cxi.s3df-atto-4bifpn512-1.0.log" &
JOB3_PID=$!
